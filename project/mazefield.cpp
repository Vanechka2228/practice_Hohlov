...
void MazeField::draw()
{
    ...
    for (int r = 0; r < rows; ++r)
    {
        for (int c = 0; c < cols; ++c)
        {
            ...
            if (cell.topWall)
                addLine(x, y, x + m_cellSize, y);
            if (cell.bottomWall)
                addLine(x, y + m_cellSize,x + m_cellSize, y + m_cellSize);
            if (cell.leftWall)
                addLine(x, y, x, y + m_cellSize);
            if (cell.rightWall)
                addLine(x + m_cellSize, y, x + m_cellSize, y + m_cellSize);
        }
    }
